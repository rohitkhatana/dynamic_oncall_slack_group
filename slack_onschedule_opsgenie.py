...
class Slack:
    def __init__(self):
        ...
        self._headers = {
            "Authorization": "Bearer {}".format(config.get("slack_auth_token"))
        }
    ...
    def __filter_group_id_by_name(self, slack_api_data, group_name):
        slack_group = list(
            filter(lambda x: x.get("name") == group_name, slack_api_data)
        )
        if not slack_group:
            return None
        else:
    ...
    def __slack_group_id(self, group_name):
        ...
        if user_group_info.get("error", None) == "name_already_exists":
            ...
            group_list_response = requests.get(
                get_group_list_endpoint, headers=self._headers
            )
    ...
    def create_update_slack_group(self, opsgenie_on_call_users):
        ...
        print(group_name, usergroup_id)
        print(on_call_participants)
        if usergroup_id:
    ...
    def __update_user_group(self, usergroup_id, on_call_participants):
        ...
        params = {"usergroup": usergroup_id, "users": slack_user_ids}
...
class Opsgenie:
    ...
    def __oncall(self, schedule_id):
        endpoint = self._opsgenie_base_url + "/schedules/{}/on-calls".format(
            schedule_id
        )
    ...
    def get_schedule_ids(self):
        return list(
            map(lambda schedule: schedule["id"], self.__get_schedules()["data"])
        )
